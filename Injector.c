...
int inject_ptrace(pid_t pid, const char *lib_path) {
    ...
    unsigned long dlopen_offset = get_func_offset("libc.so.6", "__libc_dlopen_mode");
    if (!dlopen_offset) {
    ...
    for (size_t i = 0; i < path_len; i += sizeof(long)) {
        long word = 0;
        size_t copy_len = (path_len - i < sizeof(long)) ? path_len - i : sizeof(long);
...
int inject_nsenter(pid_t pid, const char *lib_path) {
    ...
    char cp_cmd[512];
    snprintf(cp_cmd, sizeof(cp_cmd), "cp '%s' '%s' && chmod 755 '%s'", lib_path, tmp_lib, tmp_lib);
    if (system(cp_cmd) != 0) {
    ...
    if (maps) {
        char maps_line[512];
        while (fgets(maps_line, sizeof(maps_line), maps)) {
            if (strstr(maps_line, "sirracha_inject") || strstr(maps_line, "sober_test")) {
                fclose(maps);
...
int main(int argc, char **argv) {
    ...
    if (realpath(lib_path, abs_path) == NULL) {
        ...
        snprintf(container_path, sizeof(container_path), "/proc/%d/root%s", pid, lib_path);
