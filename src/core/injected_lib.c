...
int resolve_functions(luau_api_t *api) {
  ...
  if (libloader_base) {
    ...
    api->loadbuffer = (luaL_loadbuffer_t)(libloader_base + 0x13770);
    api->pcall = (lua_pcall_t)(libloader_base + 0x16db0);
    if (api->gettop)
      resolved++;
    if (api->settop)
      resolved++;
...
int execute_script(luau_api_t *api, const char *script, char *output,
  ...
  if (!api->loadbuffer) {
    snprintf(output, output_size,
             ...
             "Check the debug log at /tmp/linusware_debug.log",
  ...
  if (sigsetjmp(g_jmp, 1) == 0) {
    ...
    status = api->loadbuffer(api->L, script, strlen(script), "@linusware");
...
int luau_api_init(luau_api_t *api) {
  ...
  log_debug("===========================================\n");
  log_debug("LinusWare Executor v2.0 - Full Edition\n");
  log_debug("PID: %d\n", getpid());
  log_debug("===========================================\n");
      ...
      if (script && strlen(script) > 0) {
        ...
        if (strcmp(script, "__PROBE__") == 0) {
          ...
          write_output("Discovery complete: %d potential functions found.\n"
                       "See /tmp/linusware_debug.log for details.",
                       found);
          ...
          write_output("Safe probe complete: %d functions found.\n"
                       "Check /tmp/linusware_debug.log for offsets.",
                       found);
